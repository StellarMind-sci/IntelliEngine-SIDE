    ...
    for index, step in enumerate(steps):
        ...
        if kind == "verification":
            if not step["acceptance_statement"] or not ordered(step["evidence_node_refs"], ref_key) or not step["evidence_node_refs"]:
                return verdict(False, issue("thoughtflow.invalid_step", f"/steps/{index}"))
            used_cn.update(ref_key(value) for value in step["evidence_node_refs"])
        if kind == "iteration":
            ...
            if any(value not in by_id or by_id[value]["kind"] != "verification" for value in step["verification_step_ids"]):

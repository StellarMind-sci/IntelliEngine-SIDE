...
<body><main><h1>KnowledgeUnit 验证证据预览</h1><div class="meta"><span class="pill">mode: preview</span><span class="pill">side_effects: forbidden</span><span class="pill">focus: ${focus}</span></div><div class="state ${stateClass}">state: ${escapeHtml(result.state)}</div><p class="notice">仅验证解释与导航提示，不执行、不写入，不表达个人掌握结论。</p>${navigation(result)}${evidenceRows(result)}</main></body></html>`;
}
